 ...
 */
void _wasm_rt_set_unwind_target(WASM_RT_UNWIND_TARGET* target);
// snoopj
#define wasm_rt_set_unwind_target(target) \
	/* printf("%s:%d wasm_rt_set_unwind_target, target=%p\n", __FILE__, __LINE__, target) */ \
	_wasm_rt_set_unwind_target(target)
